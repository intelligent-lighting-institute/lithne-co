...
XBee::XBee(): _response(XBeeResponse()) 
{
	/* Added for Lithne */
	_XBeePacketSize = 0;
    _packNum = 0; 			// alternative for pos that includes escaped chars	
	for (int i = 0; i < MAX_XBEE_PACK_BYTES; i++)
	{
		_XBeePacket[i] = 0;
	}
...
/* Added for Lithne */
uint8_t XBee::getXBeePacketSize()
{
	return _XBeePacketSize;
}

uint8_t * XBee::getXBeePacket()
{
	return _XBeePacket;
}
...
void XBee::readPacket() {
    ...
    while (available()) {
		...
		/* Added for Lithne  to enable forwarding of complete packages */
		_XBeePacket[_packNum] = b;
        _packNum++;
...
void XBee::sendBytePublic(uint8_t b, bool escape) {									/////////////
																					/////////////
	if (escape && (b == START_BYTE || b == ESCAPE || b == XON || b == XOFF)) {		/////////////
//		std::cout << "escaping byte [" << toHexString(b) << "] " << std::endl;		/////////////
		write(ESCAPE);																/////////////
