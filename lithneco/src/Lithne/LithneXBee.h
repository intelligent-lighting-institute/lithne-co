...
public:
	...
	/* Added for Lithne */
	#define MAX_XBEE_PACK_BYTES 100
	uint8_t _XBeePacketSize;
    uint8_t _packNum; 			// alternative for pos that includes escaped chars	
	uint8_t _XBeePacket[MAX_XBEE_PACK_BYTES];
	
	uint8_t getXBeePacketSize();
	uint8_t * getXBeePacket();
	void sendBytePublic( uint8_t b, bool escape );
